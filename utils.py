...
divide_chain_difficulty = pickle.load(open("divide_chain_difficulty.pkl", "rb"))
...
def set_globals(_coefs: tuple[float], special_coefs: tuple[float] = (1.2,)) -> None:
    ...
    :param special_coefs: uncapped modifiers to use (Fly Upwards/Downwards, Faster Projectiles)
    :return: None
    """
    global log_coefs, increasers, decreasers, coefs, extended_coefs, extension_idx, increaser_candidates, decreaser_candidates
    coefs = _coefs
    extension_idx, extended_coefs = extend_coefs(coefs, special_coefs)
...
def choose_wisely(node, target) -> set[tuple[int]]:
    ...
    if coefs[0] < relation < coefs[-1]:
        ...
        for amount, candidate in choose_increase_decrease(node, target):
            ...
            amounts_candidates = amounts_candidates.union({(amount, candidate), (amount + 1, candidate)})
...
@functools.cache
def path_cost(value: float, path: tuple[int], target: float, budget_weight:float, specials_idx:int, acc_cap:float=np.inf) ->  float:
    """
...
def to_equation(sol: tuple[int,tuple[int]], distance:int, target:float, budget_weight:float, specials_idx:int) -> str:
    """
...
def print_results(
    ...
    for sol in solutions[:n]:
        print(
            to_equation(
                sol, distance, target, budget_weight, extension_idx
            )
        )



def extend_coefs(_coefs, special_coefs=(1.2,))-> tuple[int, tuple]:
    """
    ...
    :return:
    """
    extended_coefs = tuple(list(_coefs) + list(special_coefs))
    return len(_coefs), extended_coefs


def sort_solutions(tree: dict[float:set[tuple[int]]], scorer: callable, reverse: bool=False) -> list[tuple]:
    """
...
def get_starting_nodes(min_flyups: int, fly_up_multipliers: list[float])-> set[tuple[float, tuple]]:
    """
...
def get_multiplier(path: tuple[int]) -> float:
    """
    calculates speed multiplier from a given list of modifier amounts
    :param path: modifier amounts
    :return: speed multiplier
    """
    return float(np.prod([c ** path[i] for i, c in enumerate(extended_coefs)], dtype=np.float32))
...
def calculate_fly_up_multipliers(target_multiplier: float, max_len: int=200) -> tuple[int, np.array]:
    """
    ...
    while True:
        cnt += 1
        res = res * 1.2
        if 20 >= target_multiplier / res >= epsilon:
            if min_count is None:
                min_count = cnt
            fly_up_multipliers.append(res)
        elif target_multiplier / res < epsilon:
            break
        if len(fly_up_multipliers) >= max_len:
            break
    fly_up_multipliers = np.flip(np.asarray(fly_up_multipliers))
    return min_count, fly_up_multipliers
